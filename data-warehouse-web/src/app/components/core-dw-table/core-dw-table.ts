...
import { ICoreDwColumn } from '../../../classes/Common/ICoreDwColumn';
...
@Component({
  selector: 'app-core-dw-table',
  templateUrl: './core-dw-table.html',
  imports: [
    ButtonModule,
    ConfirmDialog,
    DatePipe,
  ...
  styleUrl: './core-dw-table.css'
})
export class CoreDwTable
 {
    ...
    @Input() tableDataKey: string = "";
    @Input() rowData!: ICoreDwRow[];
    @Input() clonedRowData!: ICoreDwRow[];
    @Input() selectedRow!: ICoreDwRow;
    @Input() columns!: ICoreDwColumn[];
    @Input() selectedColumns: ICoreDwColumn[] = this.columns;
    ...
    loadEvents() {
        this.pokemonService.getPokemonEvents().subscribe({
          next: (data: PokemonEvent[]) => {
            this.rowData = data;
            this.clonedRowData = [... data];
            this.changeDetector.markForCheck();
            console.log('Events loaded successfully: ', this.rowData);
          },
          error: (error: any) => {
    ...
    onRowEditInit(pokemonEvent: PokemonEvent) {
        this.clonedRowData[pokemonEvent.eventID] = { ...pokemonEvent };
    }
    ...
    onRowEditSave(pokemonEvent: PokemonEvent) {
       if (pokemonEvent.isNew) {
        ...
        //if (pokemonEvent.price > 0) {
            const i = this.clonedRowData.findIndex(e => e.eventID == pokemonEvent.eventID);
            delete this.clonedRowData[i];
    ...
    onRowEditCancel(pokemonEvent: PokemonEvent) {
        const i = this.rowData.findIndex(e => e.eventID == pokemonEvent.eventID);
        this.rowData[i] = pokemonEvent;
        delete this.clonedRowData[pokemonEvent.eventID];
    }

    addRow() {
      const lastIndex = this.rowData.length - 1;
      const newRow: PokemonEvent = {
        eventID: this.rowData[lastIndex].eventID + 1,
        eventName: '',
        isEventActive: true,
        startDate: new Date(),
        endDate: new Date(),
